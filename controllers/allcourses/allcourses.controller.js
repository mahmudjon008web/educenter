...
const AllCourses = db.AllCourses

const getAllcourses = async (req, res)=>{
    try {
        const user = await req.user
        const allcourses = await AllCourses.findAll({where: {id: user.id}}) 
...
const postAllcourses = async (req, res)=>{
    try {
        ...
        const newallcourses = await AllCourses.create({course_name, imgUrl})
        res.status(201).json({
...
const updateAllcourses = async (req, res)=>{
    try {
        ...
        await AllCourses.update({course_name, imgUrl}, {where: {id: user.id}})
        res.status(201).json({
