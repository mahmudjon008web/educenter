...
const getCoursetype = async (req, res)=>{
    try {
        ...
        const coursetype = await CourseType.findAll({where: {id: user.id}})
...
const postCourseType = async (req, res)=>{
    try {
        ...
        const newcoursetype = await CourseType.create({teacher, imgUrl})
        res.status(201).json({
...
const updateCoursetype = async (req, res)=>{
    try {
        ...
        await CourseType.update({teacher, imgUrl},{where: {id: user.id}})
        res.status(201).json({
